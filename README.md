...
| `npm run test:concurrency` | Re-runs the concurrency and idempotency suites **20 times each**. Concurrency defects are non-deterministic, so a suite that passed once has not been shown to pass. Runs in CI too. Verified to catch a real flake before being trusted — see [docs/AAR-M1b.md](docs/AAR-M1b.md) P1. |
| `npm run bench` | Cold-scan benchmark. Generates real media with ffmpeg, scans it at pool sizes 1–16, and prints per-file timings, a fitted pipeline model and a 20,000-file projection. Takes about a minute; excluded from the normal test run. |
| `npm run bench -- --dir "D:\Movies"` | Benchmarks your **actual library** instead of generated clips — better evidence, and it needs only ffprobe. The folder is read only. |
...
| `npm run check` | `typecheck` + `lint` + `test` + `test:concurrency`. The same four steps CI runs on every push. |
...
| `npm run scan -- --concurrency=4` **(M2)** | Overrides how many ffprobe processes run at once. Defaults to your core count, capped at 8; measured, anything past about 4 makes almost no difference, because the limit is Que's own per-file work rather than the probes. |
...
| **[docs/OPEN-ACTIONS.md](docs/OPEN-ACTIONS.md)** | Every action carried out of a review, with its current state. AARs are frozen; this is the mutable index into them. |
| **[docs/HANDOFF.md](docs/HANDOFF.md)** | Context for picking the project up: what is unverified on Windows, the traps already paid for, and what M1c needs. |
